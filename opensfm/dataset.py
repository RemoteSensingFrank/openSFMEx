...
class DataSet(object):
    ...
    def create_geo_correct_dir(self, filename=None):
        """Return dir of geo correct"""
        dir=os.path.join(self.data_path, filename or 'geo')
    ...
    def load_tracks_geocorrect(self, filename=None):
        """Return geo correct format of tracks"""
        with io.open_rt(self._tracks_graph_file(filename)) as fin:
            return tracking.load_tracks_geo(fin)        

    def tracks_exists(self, filename=None):
        return os.path.isfile(self._tracks_graph_file(filename))

    def load_track_graph(self, filename=None):
        with io.open_rt(self._tracks_graph_file(filename)) as fin:
            return tracking.load_tracks_graph(fin)
