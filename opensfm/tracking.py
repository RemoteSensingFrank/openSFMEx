...
'''
    将track格式转换为方便计算三维点的格式进行处理
    author：Frank.Wu
'''
def load_tracks_geo(fileobj):
    version = _tracks_file_version(fileobj)
    return _load_tracks_geo(fileobj)
...
#将数据以方便进行几何校正的模式读取
def _load_tracks_geo(fileobj):
    imageNode={}
    itemSave = []

    for line in fileobj:
        image, track, observation, x, y, scale, R, G, B = line.split('\t')
        itemSave.append([image, track, observation, x, y, scale, R, G, B])
        imageNode[image]=[]

    for item in itemSave:
        imageNode[item[0]].append([item[1],item[2],item[3],item[4],item[5]])
