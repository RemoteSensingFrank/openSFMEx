...
@monitor.errorhandler(psutil.NoSuchProcess)
def no_such_process(e):
    return render_template('404.html'), 404


@monitor.app_errorhandler(404)
...
@monitor.route('/cpu', defaults={'chart': None})
@monitor.route('/cpu/<string:chart>')
def cpu_info(chart):
...
@monitor.route('/memory/', defaults={'part': 'memory', 'chart': None})
@monitor.route('/memory/<string:part>')
@monitor.route('/memory/<string:part>/<string:chart>')
...
@monitor.route('/disks/', defaults={'part': 'disk', 'sort': 'space_free', 'order': 'desc'})
@monitor.route('/disks/<string:part>')
@monitor.route('/disks/<string:part>/<string:sort>')
@monitor.route('/disks/<string:part>/<string:sort>/<string:order>')
...
@monitor.route('/network/', defaults={'part': 'interfaces'})
@monitor.route('/network/<string:part>')
def network(part):
    if part == 'interfaces':
...
@monitor.route('/processes', defaults={'sort': 'cpu', 'order': 'desc'})
@monitor.route('/processes/<string:sort>')
@monitor.route('/processes/<string:sort>/<string:order>')
@monitor.before_app_first_request
...
@monitor.route('/process/<int:pid>/', defaults={'part': 'process'})
@monitor.route('/process/<int:pid>/<string:part>')
...
@monitor.route('/api', defaults={'part': 'cpu', 'chart': 'line'})
@monitor.route('/api/<string:part>/')
@monitor.route('/api/<string:part>/<string:chart>')
def api(part, chart):
    if part == 'cpu':
        cpu_info = {}
        cpu_time_percent = psutil.cpu_times_percent()
