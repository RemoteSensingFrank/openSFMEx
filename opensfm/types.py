'''
@name: 这里对精度要求太高了导致在解算过程中可能出现精度不够的情况，我把精度调低了一些
@msg: line 69 set_rotation_matrix 函数
@date: 2020-03-14
'''
...
class Pose(object):
    ...
    def set_rotation_matrix(self, rotation_matrix, permissive=False):
        ...
        if not permissive:
            if not np.isclose(np.linalg.det(R), 1,1e-4):
                raise ValueError("Determinant not 1")
            if not np.allclose(np.linalg.inv(R), R.T,1e-4):
...
class Reconstruction(object):
    ...
    def add_reference_lla(self,lla):
        self.reference_lla=lla
